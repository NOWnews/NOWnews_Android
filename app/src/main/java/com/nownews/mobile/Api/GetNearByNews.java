...
public class GetNearByNews implements Runnable {

    private final String TAG = getClass().getSimpleName();
    private Handler mHandler;
    private double mLongitude, mLatitude;

    public GetNearByNews(Handler aHandler, double aLongitude, double aLatitude) {
        mHandler = aHandler;
        mLongitude = aLongitude;
        mLatitude = aLatitude;
    }

    public void setData(Handler aHandler, double aLongitude, double aLatitude){
        mHandler = aHandler;
        mLongitude = aLongitude;
        mLatitude = aLatitude;
    }
    ...
    public void run() {
        ...
        try {
            ...
            String webApiUrl = WebAPIUrl.NEAR_BY_NEWS;
            webApiUrl = String.format(webApiUrl, mLongitude, mLatitude);
...
//			if(Utility.DEBUG)Log.e(TAG, "jsonValue: " + jsonValue);
            ...
            message.what = ParameterSet.GET_NEAR_BY_NEWS_FAILED;
            if (Utility.DEBUG) Log.e(TAG, "GET_NEAR_BY_NEWS_FAILED");
            ex.printStackTrace();
