...
public class GetVideosInfo implements Runnable {

    private final String TAG = getClass().getSimpleName();
    private Handler mHandler;
    private int mNewsId;

    public GetVideosInfo(Handler aHandler, int aNewsId) {
        mHandler = aHandler;
        mNewsId = aNewsId;
    }
    ...
    public void run() {
        ...
        try {
            ...
            String webApiUrl = WebAPIUrl.VIDEOS_INFO;
...
//			if(Utility.DEBUG)Log.e(TAG, "jsonValue: " + jsonValue);
            VideosInfoJson jsonValueClb = new Gson().fromJson(jsonValue, VideosInfoJson.class);

            Utility.writeJsonToFile(TAG, jsonValue);

            message.obj = jsonValueClb;

            message.what = ParameterSet.GET_VIDEOS_INFO_DONE;
            if (Utility.DEBUG) Log.v(TAG, "GET_VIDEOS_INFO_DONE");
            ...
            message.what = ParameterSet.GET_VIDEOS_INFO_FAILED;
            if (Utility.DEBUG) Log.e(TAG, "GET_VIDEOS_INFO_FAILED");
            ex.printStackTrace();
